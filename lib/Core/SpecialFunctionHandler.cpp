...
SpecialFunctionHandler::handleSpaLoadPath(ExecutionState &state,
  ...
  // Check if there are any non-generated symbols in the log that can be
  // consumed but that come after any symbols outputted by the current
  // participant.
  // To check this, check the log in reverse and find the first of either a
  // symbol outputted by the current participant or a non-derived symbol that
  // can be consumed.
  // If performing shallow exploration, make sure the most recent participant
  // sent symbols the current participant can consume.
  bool receivingShallowSymbols = false;
  ...
  if (SPA::ShallowExploration && (!receivingShallowSymbols) &&
