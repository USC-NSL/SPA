...
void SpecialFunctionHandler::handleSpaSeedSymbol(ExecutionState &state,
  ...
  if (SPA::seedSymbolMappings.count(name) == 0) {
    klee_message("Input %s is not connected. Not seeding.", name.c_str());
    return;
  }
  ...
  if (state.senderPath->hasOutput(senderOutName)) { // Check if sender outputs this message type.
    ...
    executor.terminateStateOnError(state, "Symbol mapping not generated by sender: " + senderOutName + ".", "user.err");
  }
}
