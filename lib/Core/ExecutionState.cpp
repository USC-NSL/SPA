...
ExecutionState::ExecutionState(KFunction *kf)
    : fakeState(false), underConstrained(false), depth(0), pc(kf->instructions),
      prevPC(pc), queryCost(0.), weight(1), instsSinceCovNew(0),
      coveredNew(false), forkDisabled(false), ptreeNode(0), step_depth(0),
      filtered(false) {
  pushFrame(0, kf);
}

ExecutionState::ExecutionState(const std::vector<ref<Expr> > &assumptions)
    : fakeState(true), underConstrained(false), constraints(assumptions),
      queryCost(0.), ptreeNode(0), step_depth(0), filtered(false) {}
...
ExecutionState::ExecutionState(const ExecutionState &state)
    : fnAliases(state.fnAliases), fakeState(state.fakeState),
      underConstrained(state.underConstrained), depth(state.depth),
      pc(state.pc), prevPC(state.prevPC), stack(state.stack),
      constraints(state.constraints), queryCost(state.queryCost),
      weight(state.weight), addressSpace(state.addressSpace),
      ...
      arrayNames(state.arrayNames), shadowObjects(state.shadowObjects),
      incomingBBIndex(state.incomingBBIndex), step_depth(state.step_depth),
      filtered(state.filtered), senderPath(state.senderPath),
...
bool ExecutionState::isReplayingSpaLog() const {
  if (senderPath) {
