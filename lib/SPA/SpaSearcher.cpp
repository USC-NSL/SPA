...
klee::ExecutionState *SpaSearcher::dequeueState(klee::ExecutionState *state) {
  states.erase(std::pair<std::vector<double>, klee::ExecutionState *>(
      oldUtilities[state], state));
...
void SpaSearcher::filterState(klee::ExecutionState *state, unsigned int id) {
  ...
  for (std::vector<FilteringEventHandler *>::iterator
           hit = filteringEventHandlers.begin(),
           hie = filteringEventHandlers.end();
       hit != hie; hit++)
...
//
// 		for ( std::set<std::pair<std::vector<double>,klee::ExecutionState *>
// >::iterator it = states.begin(), ie = states.end(); it != ie; it++ ) {
...
// 		}
//
// 		cloud9::ExecutionPathSetPin paths = jobManager->getTree()->buildPathSet(
// nodes.begin(), nodes.end(), (std::map<cloud9::worker::WorkerTree::Node
// *,unsigned> *) NULL );
//
...
// 			for ( std::vector<int>::const_iterator it = path.begin(), ie = path.end();
// it != ie; it++ ) {
// 				stateFile << *it;
// 			}
// 			stateFile << std::endl;
...
klee::ExecutionState &SpaSearcher::selectState() {
  ...
  if ((!states.begin()->second->filtered) && states.size() > QUEUE_LIMIT) {
    ...
    for (std::set<std::pair<std::vector<double>,
                            klee::ExecutionState *> >::reverse_iterator it =
             states.rbegin();
         n-- > 0; it++)
  ...
  // 		if ( SaveState.size() > 0 && difftime( time( NULL ), lastSaved ) >
  // SAVE_STATE_PERIOD ) {
  // 			saveStates();
  // 			lastSaved = time( NULL );
  // 		}
...
void SpaSearcher::printStats() {
  ...
  if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last)
          .count() > PRINT_STATS_PERIOD) {
    last = now;
    klee::klee_message(
        "[SpaSearcher] Queued: %ld; Utility Range: [%s; %s]; Processed: %ld; "
        "Filtered: %ld",
        states.size(),
