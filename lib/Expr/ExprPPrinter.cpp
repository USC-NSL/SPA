...
void PPrinter::printUpdateList(const UpdateList &updates, PrintContext &PC) {
...
void PPrinter::printWidth(PrintContext &PC, ref<Expr> e) {
...
bool PPrinter::isReadExprAtOffset(ref<Expr> e, const ReadExpr *base, ref<Expr> offset) {
...
/// null.
const ReadExpr* PPrinter::hasOrderedReads(ref<Expr> e, int stride) {
...
bool PPrinter::hasAllByteReads(const Expr *ep) {
  switch (ep->kind) {
    Expr::Read: {
// right now, all Reads are byte reads but some
// transformations might change this
...
void PPrinter::printRead(const ReadExpr *re, PrintContext &PC, unsigned indent) {
  print(re->index, PC);
  printSeparator(PC, isVerySimple(re->index), indent);
  printUpdateList(re->updates, PC);
}

void PPrinter::printExtract(const ExtractExpr *ee, PrintContext &PC, unsigned indent) {
  PC << ee->offset << ' ';
  print(ee->expr, PC);
}

void PPrinter::printExpr(const Expr *ep, PrintContext &PC, unsigned indent, bool printConstWidth) {
...
PPrinter::PPrinter(llvm::raw_ostream &_os) : os(_os), newline("\n") {
  reset();
}
...
void PPrinter::print(const ref<Expr> &e, unsigned level) {
  PrintContext PC(os);
  PC.pos = level;
  print(e, PC);
}

void PPrinter::printConst(const ref<ConstantExpr> &e, PrintContext &PC, 
                bool printWidth) {
...
void PPrinter::print(const ref<Expr> &e, PrintContext &PC, bool printConstWidth) {
...
void PPrinter::printSeparator(PrintContext &PC, bool simple, unsigned indent) {
