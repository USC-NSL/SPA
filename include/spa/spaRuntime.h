...
#define spa_msg_output_var( var ) spa_msg_output( &var, sizeof( var ), sizeof( var ), #var )
...
extern "C" {
	...
	void __attribute__((noinline,weak)) spa_input( void *var, size_t size, const char varName[], uint8_t ***initialValue, const char initialValueName[] ) {
		...
		if ( *initialValue ) {
			...
			size_t i;
			klee_assert( (*initialValue)[0] && (*initialValue)[1] && "Exhausted initial values." );
			for ( i = 0; i < size; i++ ) {
...
void __attribute__((weak)) __spa_output( void *var, size_t size, size_t maxSize, const char *varName, const char *sizeName ) {
...
#define spa_api_input( var, size, name ) printf( "[SPA] Received API Input: %s.\n", name )
#define spa_api_input_var( var ) printf( "[SPA] Received API Input: %s.\n", #var )
#define spa_state( var, size, name )
#define spa_state_var( var )
#define spa_api_output( var, size, maxSize, name ) printf( "[SPA] Sending API Output: %s.\n", name )
#define spa_api_output_var( var ) printf( "[SPA] Sending API Output: %s.\n", #var )
#define spa_msg_input( var, size, name ) printf( "[SPA] Received message: %s.\n", name )
#define spa_msg_input_size( var, name )
#define spa_msg_input_var( var ) printf( "[SPA] Received message: %s.\n", #var )
#define spa_msg_output( var, size, maxSize, name ) printf( "[SPA] Sending message: %s.\n", name )
#define spa_msg_output_var( var ) printf( "[SPA] Sending message: %s.\n", #var )
