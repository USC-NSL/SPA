...
rm -f badinputs-*.paths badinputs-*.txt

spaSplitPaths -i $1 -o 'badinputs-%06d.paths' -n 1000

parallel --progress --eta --joblog $2.joblog --resume \
  ...
  date '+Finished: %s.%N (%c)';" \
  ::: badinputs-*.paths 2>&1 | tee -a $2.log
