...
void runServer(short port) {
...
#ifndef ENABLE_KLEE
  int optval = 1;
  assert(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval,
                    sizeof(optval)) == 0);
#endif
  ...
  for (;;) {
    ...
    assert((cli_sockfd =
                accept(sockfd, (struct sockaddr *)&cliaddr, &socklen)) >= 0);
    assert(socklen == sizeof(cliaddr));

    ssize_t msglen;
    assert((msglen = recv(cli_sockfd, msg, sizeof(msg), 0)) >= 0);
    ...
    *((value_t *)&resp[sizeof(key_t)]) = value;
    send(cli_sockfd, resp, sizeof(resp), 0);
