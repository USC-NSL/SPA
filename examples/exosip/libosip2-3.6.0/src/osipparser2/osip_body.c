...
int osip_body_clone(const osip_body_t * body, osip_body_t ** dest)
{
	...
	i = osip_list_clone(body->headers, copy->headers,
...
int osip_body_parse(osip_body_t * body, const char *start_of_body, size_t length)
{
	if (body == NULL)
		return OSIP_BADPARAMETER;
...
osip_body_parse_mime(osip_body_t * body, const char *start_of_body, size_t length)
{
	...
	if (end_of_osip_body_header - start_of_osip_body_header <= 0)
		return OSIP_SYNTAXERROR;
	body->body =
		(char *) osip_malloc(end_of_osip_body_header - start_of_osip_body_header +
							 1);
	if (body->body == NULL)
		return OSIP_NOMEM;
...
int osip_body_to_str(const osip_body_t * body, char **dest, size_t * str_length)
{
	char *tmp_body;
	char *tmp;
	char *ptr;
