...
int osip_from_init(osip_from_t ** from)
{
...
void osip_from_free(osip_from_t * from)
{
	if (from == NULL)
		return;
	if (from->url != NULL) {
...
/* returns -1 on error. */
int osip_from_parse(osip_from_t * from, const char *hvalue)
{
...
/* returns -1 on error. */
int osip_from_to_str(const osip_from_t * from, char **dest)
{
	char *url;
	char *buf;
	int i;
...
char *osip_from_get_displayname(osip_from_t * from)
{
	if (from == NULL)
		return NULL;
...
void osip_from_set_displayname(osip_from_t * from, char *displayname)
{
	from->displayname = displayname;
}

osip_uri_t *osip_from_get_url(osip_from_t * from)
{
	if (from == NULL)
		return NULL;
	return from->url;
...
void osip_from_set_url(osip_from_t * from, osip_uri_t * url)
{
	from->url = url;
}
...
int osip_from_clone(const osip_from_t * from, osip_from_t ** dest)
{
	int i;
	osip_from_t *fr;
	...
	*dest = NULL;
	...
	i = osip_list_clone(&from->gen_params, &fr->gen_params,
						(int (*)(void *, void **)) &osip_generic_param_clone);
	if (i != 0) {
		osip_from_free(fr);
		return i;
	}
...
int osip_from_compare(osip_from_t * from1, osip_from_t * from2)
{
	char *tag1;
	char *tag2;
...
char *osip_generic_param_get_name(const osip_generic_param_t * fparam)
{
	if (fparam == NULL)
		return NULL;
	return fparam->gname;
...
void osip_generic_param_set_name(osip_generic_param_t * fparam, char *name)
{
	fparam->gname = name;
}

char *osip_generic_param_get_value(const osip_generic_param_t * fparam)
{
	if (fparam == NULL)
		return NULL;
