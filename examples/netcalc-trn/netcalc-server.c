...
void handleQuery(nc_query_t &query, nc_response_t &response) {
  ...
  // 		std::cout << query.arg1 << " " << getOpName( query.op ) << " " <<
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    assert(recvfrom(s, &query, sizeof(query), 0, (struct sockaddr *)&si_client,
                    &si_client_len) == sizeof(query));
    ...
    assert(sendto(s, &response, sizeof(response), 0,
