...
#ifndef ENABLE_KLEE
    assert(sendto(sock, &capabilityRequest, sizeof(capabilityRequest), 0,
                  server->ai_addr, server->ai_addrlen) ==
           sizeof(capabilityRequest));
    assert(recv(sock, &capabilityResponse, sizeof(capabilityResponse), 0) ==
...
nc_value_t executeQuery(nc_operator_t op, nc_value_t arg1, nc_value_t arg2) {
  ...
  assert(op > NC_CAPABILITY && op <= capability &&
         "Invalid operator in query.");
  // 	assert( (op == NC_CAPABILITY || (op >= 0 && op <  capability)) && "Invalid
  // operator in query.");
...
#ifndef ENABLE_KLEE
  // Send query.
  assert(sendto(sock, &query, sizeof(query), 0, server->ai_addr,
                server->ai_addrlen) == sizeof(query));
  // Get response.
  ...
  // 			std::cerr << "	" << getOpName( query.op ) << " = " << response.value <<
  // std::endl;
  // 		else
