...
llvm::cl::opt<std::string>
    OutputPattern("o", llvm::cl::desc("Output file name pattern."),
                  llvm::cl::Required);

llvm::cl::opt<unsigned> NumOutFiles("n",
                                    llvm::cl::desc("Number of output files."));
...
void addOutput(unsigned long index, std::string text) {
  ...
  assert(snprintf(fileName, sizeof(fileName), OutputPattern.getValue().c_str(),
                  index + 1) > 0);

  std::ofstream ofs(fileName,
                    std::ios_base::out |
                        (initializedFiles.count(index) ? std::ios_base::app
                                                       : std::ios_base::trunc));
...
int main(int argc, char **argv, char **envp) {
  ...
  llvm::cl::ParseCommandLineOptions(
      argc, argv, "Systematic Protocol Analyzer - Path-file Splitter");

  assert((NumOutFiles.getValue() > 0) != (NumPathsPerFile.getValue() > 0) &&
         "Either specify -n or -p.");
  ...
  if (NumOutFiles.getValue() > 0) {
    ...
    numPathsPerFile =
        std::min(pathCount, (unsigned long) NumPathsPerFile.getValue());
    numFiles = ceil(((double) pathCount) / NumPathsPerFile.getValue());
  }

  llvm::outs() << "Created " << numFiles << " path-files with up to "
               << numPathsPerFile << " paths per file.\n";
}
