...
int main(int argc, char **argv, char **envp) {
  ...
  llvm::cl::ParseCommandLineOptions(
      argc, argv, "Systematic Protocol Analyzer - Request Response");
  ...
  if (Client) {
    ...
    if (fn) {
      ...
      klee::klee_message(
          "   Message annotation function not present in module.");
    }
  } else if (Server) {
    llvm::Function *fn =
  ...
  for (std::set<llvm::Instruction *>::iterator it = checkpoints.begin(),
                                               ie = checkpoints.end();
       it != ie; it++)
  ...
  SPA::CFGBackwardFilter *filter =
      new SPA::CFGBackwardFilter(cfg, cg, checkpoints);
  // 	if ( Client )
  // 		spa.addStateUtilityBack( filter, false );
  // 	else if ( Server )
  spa.addStateUtilityBack(filter, true);
  for (std::set<llvm::Instruction *>::iterator it = entryPoints.begin(),
                                               ie = entryPoints.end();
       it != ie; it++) {
  ...
  if (Client) {
    // 		spa.addStateUtilityBack( new SPA::AstarUtility( cfg, cg, checkpoints ),
    // false );
  ...
  if (DumpCFG.size() > 0) {
    ...
    cfg.dump(dotFile, cg, /*filter*/ NULL, annotations,
