...
void processPaths( SPA::Path *cp, SPA::Path *sp ) {
	...
	if ( (cp && cp->getTag( SPA_VALIDPATH_TAG ) == SPA_VALIDPATH_VALUE) && sp->getTag( SPA_VALIDPATH_TAG ) == SPA_VALIDPATH_VALUE ) {
  ...
  if (ClientPathFile.size() > 0) {
    ...
    while ( true ) {
      ...
      if ( pid == 0 ) { // Child process: work coordinator.
        ...
        if ( restoreCheckpoint( cpFileName, clientPath, serverPath, processed, gd->solutions, totalClientPaths, totalServerPaths, gd->cpuTime ) ) {
          LOG() << "Restoring server path." << std::endl;
          assert(serverPathLoader.skipPaths(serverPath) && "Unable to load server path file to checkpoint position.");
        } else {
          ...
          for (totalServerPaths = 0; serverPathLoader.skipPath(); totalServerPaths++);
        ...
        do {
          do {
            ...
            if (sp) {
              LOG() << "Processing server path " << (serverPath + 1) << "/" << totalServerPaths << " - " << (serverPath * 100 / totalServerPaths) << "%). Found " << gd->solutions << " solutions." << std::endl;
