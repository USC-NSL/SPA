...
#define LOG() std::cerr << "[" << difftime(time(NULL), programStartTime) << "] "

namespace {
llvm::cl::opt<std::string>
    ClientPathFile("client", llvm::cl::desc("Specifies the client path file."));

llvm::cl::opt<std::string>
    ServerPathFile("server", llvm::cl::desc("Specifies the server path file."));

llvm::cl::opt<std::string>
    OutputFile("o",
               llvm::cl::desc("Specifies the file to write output data to."));

llvm::cl::opt<std::string>
    DebugFile("d",
              llvm::cl::desc("Specifies the file to write debug data to."));

llvm::cl::opt<bool>
    Follow("f", llvm::cl::desc("Follows input files as they are generated."));

llvm::cl::opt<int>
    NumWorkers("p", llvm::cl::desc("Number of parallel workers to create."));

llvm::cl::opt<int>
    NumJobsPerWorker("j",
                     llvm::cl::desc("Number of jobs to give to each worker."));

llvm::cl::opt<int>
    ExpansionFrontWidth("w",
                        llvm::cl::desc("The width of the expansion front."));
}
...
unsigned int numWorkers, numJobsPerWorker, expansionFrontWidth,
    runningWorkers = 0;
...
// 	for ( klee::ConstraintManager::const_iterator it = cm.begin(), ie =
// cm.end(); it != ie; it++ )
// 		LOG() << *it << std::endl;
// }
...
void processPaths(SPA::Path *cp, SPA::Path *sp) {
  ...
  if ((cp && cp->getTag(SPA_VALIDPATH_TAG) == SPA_VALIDPATH_VALUE) &&
      sp->getTag(SPA_VALIDPATH_TAG) == SPA_VALIDPATH_VALUE) {
  ...
  if (cp)
    for (klee::ConstraintManager::const_iterator
             it = cp->getConstraints().begin(),
             ie = cp->getConstraints().end();
         it != ie; it++)
      cm.addConstraint(*it);
  // Add server path constraints.
  for (klee::ConstraintManager::const_iterator
           it = sp->getConstraints().begin(),
           ie = sp->getConstraints().end();
       it != ie; it++)
    cm.addConstraint(*it);
  ...
  if (cp) {
    for (auto it : cp->getSymbols()) {
      // Check if input.
      if (it.first.compare(0, strlen(SPA_INPUT_PREFIX), SPA_INPUT_PREFIX) ==
          0) {
        // Check if message.
        if (it.first.compare(0, strlen(SPA_MESSAGE_INPUT_PREFIX),
                             SPA_MESSAGE_INPUT_PREFIX) == 0) {
          std::string type = it.first
              .substr(strlen(SPA_MESSAGE_INPUT_PREFIX), std::string::npos);
          const klee::Array *clientIn = it.second;
          std::string serverOutName =
              std::string(SPA_MESSAGE_OUTPUT_PREFIX) + type;
          if (sp->hasOutput(serverOutName)) {
            // 					LOG() << "	Connecting server-to-client message type: " <<
            // type << std::endl;
            assert(clientIn->size >= sp->getOutputSize(serverOutName) &&
                   "Client and server message size mismatch.");
            // Add server output values = client input array constraint.
            for (size_t offset = 0; offset < sp->getOutputSize(serverOutName);
                 offset++) {
              klee::UpdateList ul(clientIn, 0);
              klee::ref<klee::Expr> e = exprBuilder->Eq(
                  exprBuilder->Read(
                      ul, exprBuilder->Constant(offset, klee::Expr::Int32)),
                  sp->getOutputValue(serverOutName, offset));
              if (!cm.addAndCheckConstraint(e)) {
                // 							LOG() << "	Constraints trivially UNSAT." << std::endl;
                // 							showConstraints( cm );
                // 							LOG() << "		New constraint:" << std::endl << e <<
                // std::endl;
                return;
              }
            }
          } else {
            // 					LOG() << "	Client consumes a message type not generated by
            // server: " << it->first << ". Leaving as unknown."<< std::endl;
            objectNames.push_back(it.first);
            objects.push_back(it.second);
          }
        } else { // Not a message (API): solve as an unknown.
          // 				LOG() << "	Free client input: " << it->first << ". Leaving as
          // unknown."<< std::endl;
          objectNames.push_back(it.first);
          objects.push_back(it.second);
        }
      ...
      std::string initOutName =
          SPA_INIT_PREFIX +
          it.first.substr(strlen(SPA_PREFIX), std::string::npos);
      if (cp->hasOutput(initOutName)) {
        // 			LOG() << "	Adding user-defined client constraints for: " <<
        // it->first << std::endl;
        const klee::Array *clientIn = it.second;
        assert(clientIn->size == cp->getOutputSize(initOutName) &&
               "Constraint size mismatch.");
        // Add server output values = client input array constraint.
        for (size_t offset = 0; offset < cp->getOutputSize(initOutName);
             offset++) {
          klee::UpdateList ul(clientIn, 0);
          klee::ref<klee::Expr> e = exprBuilder->Eq(
              exprBuilder->Read(
  ...
  for (auto it : sp->getSymbols()) {
    // Check if input.
    if (it.first.compare(0, strlen(SPA_INPUT_PREFIX), SPA_INPUT_PREFIX) == 0) {
      // Check if message.
      if (it.first.compare(0, strlen(SPA_MESSAGE_INPUT_PREFIX),
                           SPA_MESSAGE_INPUT_PREFIX) == 0) {
        // Only connect message I/O is a client path is available.
        if (cp) {
          std::string type = it.first
              .substr(strlen(SPA_MESSAGE_INPUT_PREFIX), std::string::npos);
          const klee::Array *serverIn = it.second;
          std::string clientOutName =
              std::string(SPA_MESSAGE_OUTPUT_PREFIX) + type;
          // Check if client outputs this message type.
          if (cp->hasOutput(clientOutName)) {
            // 					LOG() << "	Connecting client-to-server message type: " <<
            // type << std::endl;
            assert(serverIn->size >= cp->getOutputSize(clientOutName) &&
                   "Client and server message size mismatch.");
            // Add client output values = server input array constraint.
            for (size_t offset = 0; offset < cp->getOutputSize(clientOutName);
                 offset++) {
            ...
            // 					LOG() << "	Server consumes a message type not generated by
            // client: " << it->first << ". Leaving as unknown."<< std::endl;
            objectNames.push_back(it.first);
            objects.push_back(it.second);
          }
        }
      } else { // Not a message (API): solve as an unknown.
        // 				LOG() << "	Free server input: " << it->first << ". Leaving as
        // unknown."<< std::endl;
        objectNames.push_back(it.first);
        objects.push_back(it.second);
      }
    ...
    if (sp->hasOutput(initOutName)) {
      // 			LOG() << "	Adding user-defined server constraints for: " <<
      // it->first << std::endl;
      const klee::Array *serverIn = it.second;
      assert(serverIn->size == sp->getOutputSize(initOutName) &&
             "Constraint size mismatch.");
      // Add server output values = client input array constraint.
      for (size_t offset = 0; offset < sp->getOutputSize(initOutName);
           offset++) {
        klee::UpdateList ul(serverIn, 0);
        klee::ref<klee::Expr> e = exprBuilder->Eq(
            exprBuilder->Read(ul,
  ...
  if (solver->getInitialValues(klee::Query(cm, exprBuilder->False()), objects,
                               result)) {
...
bool saveCheckpoint(std::string cpFileName, unsigned long clientPath,
                    unsigned long serverPath, unsigned long solutions,
                    unsigned long totalClientPaths,
                    unsigned long totalServerPaths, unsigned long cpuTime) {
  ...
  if (shutdown || difftime(now, last) > CHECKPOINT_TIME) {
    ...
    cpFile << clientPath << " " << serverPath << " " << totalClientPaths << " "
           << totalServerPaths << " " << solutions << " " << cpuTime
           << std::endl;
    cpFile.flush();
    cpFile.close();
...
bool restoreCheckpoint(std::string cpFileName, unsigned long &clientPath,
                       unsigned long &serverPath, unsigned long &processed,
                       unsigned long &solutions,
                       unsigned long &totalClientPaths,
                       unsigned long &totalServerPaths,
                       unsigned long &cpuTime) {
  std::ifstream cpFile(cpFileName.c_str());
  if (cpFile.is_open()) {
    LOG() << "Restarting from checkpoint. Expect some redundant results."
          << std::endl;
    cpFile >> clientPath >> serverPath >> totalClientPaths >>
        totalServerPaths >> solutions >> cpuTime;
...
int main(int argc, char **argv, char **envp) {
  ...
  gd = (GlobalData_t *)mmap(NULL, sizeof(GlobalData_t), PROT_READ | PROT_WRITE,
  ...
  numJobsPerWorker =
      NumJobsPerWorker > 0 ? NumJobsPerWorker : DEFAULT_NUM_JOBS_PER_WORKER;

  assert(ExpansionFrontWidth >= 0 && "Invalid expansion front width.");
  expansionFrontWidth = ExpansionFrontWidth > 0 ? ExpansionFrontWidth
                                                : DEFAULT_EXPANSION_FRONT_WIDTH;

  signal(SIGINT, handleSignal);

  if (ClientPathFile.size() > 0) {
    std::ifstream clientFile;
    clientFile.open(ClientPathFile.getValue().c_str());
    assert(clientFile.good() && "Unable to open path file.");
    ...
    while (true) {
      ...
      if (pid == 0) { // Child process: work coordinator.
        ...
        if (restoreCheckpoint(cpFileName, clientPath, serverPath, processed,
                              gd->solutions, totalClientPaths, totalServerPaths,
                              gd->cpuTime)) {
          LOG() << "Restoring client path." << std::endl;
          for (unsigned long i = 1; i <= clientPath; i++)
            assert((cp = clientPathLoader.getPath()) &&
                   "Unable to load client path file to checkpoint position.");
          LOG() << "Restoring server path." << std::endl;
          for (unsigned long i = 1; i <= serverPath; i++)
            assert((sp = serverPathLoader.getPath()) &&
                   "Unable to load server path file to checkpoint position.");
        } else {
          ...
          clientPathLoader.restart();
          LOG() << "Found " << totalClientPaths << " initial client paths."
                << std::endl;
          ...
          serverPathLoader.restart();
          LOG() << "Found " << totalServerPaths << " initial server paths."
                << std::endl;

          LOG() << "Processing client path 1/" << totalClientPaths
                << " with server path 1/" << totalServerPaths << "."
                << std::endl;
        ...
        do {
          ...
          saveCheckpoint(cpFileName, clientPath, serverPath, gd->solutions,
                         totalClientPaths, totalServerPaths, gd->cpuTime);

          std::vector<SPA::Path *> expansionFrontPaths;
          while (expansionFrontPaths.size() < expansionFrontWidth &&
                 (cp = clientPathLoader.getPath())) {
          ...
          if (!expansionFrontPaths.empty()) {
            ...
            for (unsigned long i = 1; i <= serverPath; i++) {
              ...
              for (unsigned long j = 0; j < expansionFrontPaths.size(); j++) {
                ...
                LOG() << "Processing client path "
                      << (clientPath - expansionFrontPaths.size() + j + 1)
                      << "/" << totalClientPaths << " with server path " << i
                      << "/" << totalServerPaths << " (pair " << processed
                      << "/" << (totalClientPaths * totalServerPaths) << " - "
                      << (processed * 100 / totalClientPaths / totalServerPaths)
                      << "%). Found " << gd->solutions << " solutions."
                      << std::endl;
                processJob(cp, sp, i == serverPath,
                           j == expansionFrontPaths.size() - 1);
                processed++;
              }
            }
            processing = true;
          ...
          if (!expansionFrontPaths.empty()) {
            ...
            for (unsigned long i = 1; i <= clientPath; i++) {
              ...
              for (unsigned long j = 0; j < expansionFrontPaths.size(); j++) {
                ...
                LOG() << "Processing client path " << i << "/"
                      << totalClientPaths << " with server path "
                      << (serverPath - expansionFrontPaths.size() + j + 1)
                      << "/" << totalServerPaths << " (pair " << processed
                      << "/" << (totalClientPaths * totalServerPaths) << " - "
                      << (processed * 100 / totalClientPaths / totalServerPaths)
                      << "%). Found " << gd->solutions << " solutions."
                      << std::endl;
                processJob(cp, sp, j == expansionFrontPaths.size(),
                           i == clientPath);
                processed++;
              }
            }
        ...
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
          time_t now = time(NULL);
          LOG() << "Done. Found " << gd->solutions << " solutions."
                << std::endl;
          LOG() << "Elapsed Time: " << difftime(now, startTime)
                << ", CPU Time:" << gd->cpuTime << std::endl;
          break;
        }
        if (shutdown) {
  ...
  } else { // Only server path file specified (symbolically seeded).
    while (true) {
      int pid;
      assert((pid = fork()) >= 0 && "Error spawning work coordinator process.");
      if (pid == 0) { // Child process: work coordinator.
        ...
        if (restoreCheckpoint(cpFileName, clientPath, serverPath, processed,
                              ...
                              gd->cpuTime)) {
          LOG() << "Restoring server path." << std::endl;
          assert(serverPathLoader.skipPaths(serverPath) &&
                 "Unable to load server path file to checkpoint position.");
        } else {
          while ((!serverPathLoader.skipPath()) && Follow) {
            LOG() << "Server path file is empty. Sleeping." << std::endl;
            sleep(1);
          }
          serverPathLoader.restart();

          for (totalServerPaths = 0; serverPathLoader.skipPath();
               totalServerPaths++)
            ;
          assert(totalServerPaths > 0 && "Empty path file.");
          serverPathLoader.restart();
          LOG() << "Found " << totalServerPaths << " initial server paths."
                << std::endl;
        }
        ...
        do {
          do {
            ...
            if (sp) {
              LOG() << "Processing server path " << (serverPath + 1) << "/"
                    << totalServerPaths << " - "
                    << (serverPath * 100 / totalServerPaths) << "%). Found "
                    << gd->solutions << " solutions." << std::endl;
              processJob(NULL, sp, false, true);
              serverPath++;
              processed++;
            }
          } while (serverPath % expansionFrontWidth != 0);
